...
\begin{frame}{Data Architecture Implementation}
\begin{columns}[T,totalwidth=\textwidth]
\column{0.56\textwidth}
\begin{itemize}
    \item Implementation refers to putting Data Architecture into action
    \item Implementation of Data Architecture includes the below key points: 
    \begin{itemize}
    	\item Creation of Data models, data flows and roadmaps
    	\item Setting up of teams
    	\item Setting up the way of working
    	\item Using data architecture in project
    	\item Creating awareness in the organization
    \end{itemize}
    \item Implementation usually starts in the project and grows across the organization.
    \item Early project may need extra efforts and funding
\end{itemize}
...
\begin{frame}{Key elements in Implementation}
	Below are the central elements that are involved in implementation of Data Architecture:
\begin{enumerate}
	\item Metadata
	\begin{itemize}
		\item Data Architecture artifacts such as models, flows are crucial metadata
		\item Metadata helps understand data and system
		\item It is used to validate the schema
	\end{itemize}
	\item Governance Basics
	\begin{itemize}
		\item It is helpful in defining rules, standards and compliance
		\item It helps ensure project flow architecture
	\end{itemize}
	\item Data Quality
	\begin{itemize}
		\item It aims at improving the data quality and trust
		\item It ensures that inconsistent data and data copies are avoided
	\end{itemize}
\end{enumerate}
\end{frame}

\begin{frame}{MDM \& Architecture Approach}
\begin{enumerate}
	\item Master Data (MDM Concept)
	\begin{itemize}
		\item Implementation of a Data Architecture mostly starts with master data(e.g., customer, product	)
		\item This is then expanded to transaction data
	\end{itemize}
	\item Architectural Patterns\\
	There are mainly two approaches:
	\begin{itemize}
		\item Agile: step by step, incremental and flexible
		\item Traditional: Blueprints, standards are followed
	\end{itemize}
	However, mostly in the organization the hybrid approach is followed which involves both mentioned above
	\item Data Architecture ensures the below:
	\begin{itemize}
		\item Standard structure(Standardization)
		\item Integration across system (Integration)
		\item Reuse of data assets (Re-usability)
	\end{itemize}
\end{enumerate}
\end{frame}


\begin{frame}{Risk, Culture \& Success}
\begin{enumerate}
	\item Risk: The main risk involved in building a Data Architecture are:
	\begin{itemize}
		\item Lack of Management support
		\item Poor communication
		\item Resistance to change
		\item Inexperienced project leaders
	\end{itemize}
	\item Cultural Success Factors\\
	\begin{itemize}
		\item Data should be treated as business assets
		\item Thinking should be at the enterprise level
		\item Accept and follow the governance practices
	\end{itemize}
\end{enumerate}
\end{frame}

\begin{frame}{How is Data Architecture Implemented}
	Data Architecture Implementation Process:
	\begin{itemize}
		\item Data is received from \textbf{data providers (source systems)}
		\item Data enters an \textbf{incoming tier (landing zone)}
		\item \textbf{Metadata is used} to validate schema compatibility
		\item Validated data is stored in \textbf{data storage (e.g., RDS)}
		\item Approved data is made available to \textbf{data consumers}
		\item Ensures \textbf{controlled, consistent, and trusted data flow}
	\end{itemize}
	...
	\begin{minipage}[t]{0.9\textwidth}
		...
		\begin{tikzpicture}[scale=0.8, every node/.style={font=\scriptsize}]
			...
			% Nodes
			\node (p) [draw, rounded corners, fill=blue!20, minimum width=2cm, minimum height=0.8cm] 
			{Providers};
			
			\node (i) [right of=p, xshift=2cm, draw, dashed, rounded corners, fill=gray!15, minimum width=2.2cm, minimum height=1cm] 
			{Incoming};
			
			\node (s) [right of=i, xshift=2cm, draw, rounded corners, fill=green!20, minimum width=2cm, minimum height=0.8cm] 
			{RDS};
			
			\node (c) [right of=s, xshift=2cm, draw, rounded corners, fill=orange!20, minimum width=2cm, minimum height=0.8cm] 
			{Consumer};
			
			% Prominent arrows
			\draw[->, line width=1.2pt, black] (p) -- (i);
			\draw[->, line width=1.2pt, black] (i) -- (s);
			\draw[->, line width=1.2pt, black] (s) -- (c);
			
			% Metadata / validation (dashed but visible)
			\node (m) [above of=i, yshift=0.5cm] {\scriptsize Metadata + Contract Validation};
			\draw[->, dashed, line width=1pt, black] (m) -- (i);
